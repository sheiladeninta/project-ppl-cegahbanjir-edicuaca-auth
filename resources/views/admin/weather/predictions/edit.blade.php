            ...
            <form action="{{ route('admin.weather.predictions.update', $prediction) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control @error('prediction_date') is-invalid @enderror" id="prediction_date" name="prediction_date" value="{{ old('prediction_date', $prediction->prediction_date->format('Y-m-d')) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('risk_level') is-invalid @enderror" id="risk_level" name="risk_level" required>
                            ...
                            @foreach($riskLevels as $risk)
                                <option value="{{ $risk }}" {{ old('risk_level', $prediction->risk_level) == $risk ? 'selected' : '' }}>
                                    {{ ucfirst($risk) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" min="0" class="form-control @error('predicted_rainfall') is-invalid @enderror" id="predicted_rainfall" name="predicted_rainfall" value="{{ old('predicted_rainfall', $prediction->predicted_rainfall) }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3">{{ old('notes', $prediction->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
